...
class Environment:
    ...
    def reset(self):
        ...
        self._update_state()
        self._update_configuration_space()
        if self.show_occupancy_map:
    ...
    def setupOrtho(self, global_map=False):
        """
        Sets up a top-down view that simulates orthographic projection by using a small FOV and high camera height.
        """
        # Get the robot's position
        robot_x, robot_y, _ = self.robot_position

        if global_map:
            view_width = 232 / LOCAL_MAP_PIXELS_PER_METER  # Width of the view in meters
            camera_height = 50  # Large height to reduce perspective distortion
            camera_position = [0, 0, camera_height]
            camera_target = [0, 0, 0.0]  # Centered on the room
            up_vector = [0, 1, 0]  # Y-axis as up (top-down view)
        else:
            view_width = LOCAL_MAP_WIDTH*2
            # Camera position (high above the robot)
            camera_height = 50  # Large height to reduce perspective distortion
            camera_position = [robot_x, robot_y, camera_height]
            camera_target = [robot_x, robot_y, 0.0]  # Looking directly at the robot's position
            up_vector = [np.cos(self.robot_heading), np.sin(self.robot_heading), 0]  # Y-axis as up (top-down view)

        # Set a very small FOV to reduce perspective effects
        fov = (view_width / (2*camera_height)) * (180 / np.pi)  # Field of view in degrees (adjust to match physical view size)
        near_plane = 0.1
        far_plane = 100.0
    ...
    def takeRGBD(self, cam, width = LOCAL_MAP_PIXEL_WIDTH, height = LOCAL_MAP_PIXEL_WIDTH, show_image = False):
        view_matrix, projection_matrix = cam
        ...
        if show_image:
            rgb_image = np.reshape(rgb_image, (height, width, 4)).astype(np.float32) / 255.0 # (H, W, RGBA)
            rgb_image = rgb_image[:, :, :3] # (H, W, RGB)
        ...
        # Convert depth buffer to real-world distances
        near_plane = 0.01
        far_plane = 10
        depth_image = far_plane * near_plane / (far_plane - (far_plane - near_plane) * depth_buffer)

        return rgb_image, depth_image

    def step(self, action, debug=False):
        return self._step(action, debug=debug)
    ...
    def _step(self, action, dry_run=False, debug=False):
        ...
        if debug:
            ...
            increment = 360 / 4
            self.simple_action_space_turn_angles = [np.radians(i * increment) for i in range(4)]
        else:
            # Store new action
            if self.use_steering_commands:
                robot_action = action
        ...
        # straight_line_heading = restrict_heading_range(turn_angle) # Global heading
    ...
    def _segment_RGBD(self):
        # _, depth_image = self.takeRGBD(self.setupOrtho(global_map=True), 116, 116, show_image=True)
        width, height = 116, 116
        view_matrix, projection_matrix = self.setupOrtho(global_map=True)

        _, _, rgb_image, depth_buffer, seg_image = p.getCameraImage(width, height, view_matrix, projection_matrix, renderer=p.ER_BULLET_HARDWARE_OPENGL)

        # Convert depth buffer to depth values
        near_plane = 0.1
        far_plane = 100.0
        depth = far_plane * near_plane / (far_plane - (far_plane - near_plane) * depth_buffer)
        ...
        # Intrinsics
        FOV = ((232 / LOCAL_MAP_PIXELS_PER_METER) / (2 * 50)) / (180 / np.pi)
        ...
        # Get segmentation
        # seg_raw = np.reshape(seg_image, (self.camera_image_pixel_height, self.camera_image_pixel_width))
        seg_raw = seg_image
        seg = np.zeros_like(seg_raw, dtype=np.float32)
        seg += FLOOR_SEG_INDEX * (seg_raw == 0)
        seg += OBSTACLE_SEG_INDEX * (seg_raw >= self.min_obstacle_id) * (seg_raw <= self.max_obstacle_id)
    ...
    def _update_configuration_space(self):
        """
        Precomputes the configuration space for the robot
        Obstacles are dilated based on the robot's radius to define a collision-free space
        """

        points, seg = self._segment_RGBD()

        obstacle_map = self._create_padded_room_zeros()
        small_obstacle_map = np.zeros((points.shape[0], points.shape[1]), dtype=np.uint8)
        augmented_points = np.concatenate((points, seg[:, :, np.newaxis]), axis=2).reshape(-1, 4)

        # Flatten the obstacle map to match the shape of augmented_points
        small_obstacle_map_flat = small_obstacle_map.flatten()

        # Use the boolean mask to update the flattened obstacle map
        mask = np.isclose(augmented_points[:, 3], OBSTACLE_SEG_INDEX)
        small_obstacle_map_flat[mask] = 1

        # Reshape the obstacle map back to its original shape
        small_obstacle_map = small_obstacle_map_flat.reshape(small_obstacle_map.shape)
        ...
        self.configuration_space = 1 - np.maximum(self.wall_map, binary_dilation(obstacle_map, selem).astype(np.uint8))

        selem_thin = disk(np.floor(ROBOT_HALF_WIDTH * LOCAL_MAP_PIXELS_PER_METER))
        self.configuration_space_thin = 1 - np.maximum(self.wall_map, binary_dilation(obstacle_map, selem_thin)).astype(np.uint8)
        self.closest_cspace_indices = distance_transform_edt(1 - self.configuration_space, return_distances=False, return_indices=True)

    def _update_global_overhead_map(self):
        points, seg = self._segment_RGBD()

        # obstacle_map = self._create_padded_room_zeros()
        small_overhead_map = np.zeros((points.shape[0], points.shape[1]), dtype=np.float32)
        augmented_points = np.concatenate((points, seg[:, :, np.newaxis]), axis=2).reshape(-1, 4)

        # Flatten the obstacle map to match the shape of augmented_points
        small_overhead_map_flat = small_overhead_map.flatten()

        # Use the boolean mask to update the flattened obstacle map
        for i, idx in zip([1, 3, 4, 5], [FLOOR_SEG_INDEX, RECEPTACLE_SEG_INDEX, ROBOT_SEG_INDEX, CUBE_SEG_INDEX]):
            mask = np.isclose(augmented_points[:, 3], idx)
            small_overhead_map_flat[mask] = i / MAX_SEG_INDEX

        # Reshape the obstacle map back to its original shape
        small_overhead_map = small_overhead_map_flat.reshape(small_overhead_map.shape)
        ...
        self.spy = self.global_overhead_map

    def _update_state(self):
        self._update_global_overhead_map()
        return
        ...
        print(self.global_overhead_map.max(), self.global_overhead_map.min())
        self.spy = self.global_overhead_map
    ...
    def get_state(self, done=False):
        ...
        # for ax, i in zip(self.axes, range(4)):
        #     ax.clear()
        #     ax.set_title(f'Channel {i}')
        #     im = ax.imshow(channels[i], cmap='hot', interpolation='nearest')
        #     # self.fig.colorbar(im, ax=ax)
        #     if self.colorbars[i-1] is not None:
        #         self.colorbars[i-1].update_normal(im)
        #     else:
        #         self.colorbars[i-1] = self.fig.colorbar(im, ax=ax)

        # self.plt.draw()
        # self.plt.pause(0.001)  # Pause to update the figures
