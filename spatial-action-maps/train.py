# Adapted from https://github.com/jimmyyhwu/spatial-action-maps/blob/master/train.py#L24
...
def train(cfg, policy_net, target_net, optimizer, batch, transform_func):
    ...
    # print(expected_state_action_values.shape, reward_batch.shape, next_state_values.shape) # (32,32) (32,1) (32)
...
def main(cfg):
    ...
    policy = policies.SteeringCommandsPolicy(cfg=cfg, action_space=5, train=True) #acton_space = len(env.available_actions)
    ...
    for timestep in tqdm(range(start_timestep, total_timesteps_with_warm_up),
        ...
        # Step the simulation
        next_state, reward, done, info = env.step(env.available_actions[action])
